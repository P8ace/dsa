...
func TestQuickSort(t *testing.T) {
	...
	for idx, test_case := range test_cases {
		t.Run(fmt.Sprintf("Test_case_no:%d", idx+1), func(t *testing.T) {
			...
			quickSort(test_case.input, 0,len(test_case.input)-1)
			if !reflect.DeepEqual(test_case.input, test_case.expected) {
				t.Errorf("Failed Test for quick sort. Input: %v, Expected: %v, Actual: %v", original, test_case.expected, test_case.input)
			}
		})
	}
}
